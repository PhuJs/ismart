...
// if(!is_login() && get_action() != 'login' && get_action() != 'regis' && get_action() != 'active' && get_action() != 'reset' && get_action() != 'resetOk'){
//     redirect_to("?mod=users&controller=index&action=login");
// }

if(isset($_GET['search'])){
    $value = $_GET['search'];
    redirect_to("?mod=products&controller=index&action=searchPd&value={$value}");
}